...
(function(global2, factory) {
  ...
  class Resolver {
    constructor(logger = new Logger(), dnsResolver = httpResolve, initialDuration = defaultDuration) {
  ...
  class Http {
    constructor(logger, resolver, client = new HttpClient()) {
      this.logger = logger;
  ...
  class Client {
    constructor(appInfo, config) {
      ...
      this.logger = (_b = config == null ? void 0 : config.logger) != null ? _b : new Logger();
      const resolver = new Resolver(this.logger, config == null ? void 0 : config.dnsResolver);
      this.http = (_c = config == null ? void 0 : config.http) != null ? _c : new Http(this.logger, resolver);
  ...
  const useTcp = true;
  const debug$2 = (...args) => console.debug("[cdnr/http/webrtc/how]", ...args);
  class HoW {
    constructor() {
      __publicField(this, "pcMap", /* @__PURE__ */ new Map());
      __publicField(this, "pcMutex", new Mutex());
    }
    getPc(target, fingerprint) {
      return __async(this, null, function* () {
        return this.pcMutex.runExclusive(() => __async(this, null, function* () {
    ...
    fetch(ctx, id, request, fingerprint) {
      return __async(this, null, function* () {
        ...
        const pc = yield this.getPc(target, fingerprint);
        const processor = new HowRequest(pc, id, request);
...
`;
  ...
  function listen$1(dc, type, listener) {
  ...
  function streamForDC(dc) {
    ...
    return new ReadableStream({
      start: (ctrl) => {
        disposers.push(listen$1(dc, "message", ({ data }) => {
          debug$2("DataChannel message", data);
          ctrl.enqueue(data);
        }));
        disposers.push(listen$1(dc, "error", (e) => {
          debug$2("DataChannel error", e);
          dispose();
          ctrl.error(e);
        }));
        disposers.push(listen$1(dc, "close", () => {
          debug$2("DataChannel close");
          dispose();
          ctrl.close();
        }));
      },
      cancel: (reason) => {
        debug$2("dcStream canceled, reason:", reason);
        dispose();
      }
    });
  }
  class HowRequest {
    constructor(pc, id, request) {
      __publicField(this, "dc");
      this.pc = pc;
      this.id = id;
      this.request = request;
      this.dc = pc.createDataChannel(`http|${this.id}`);
    }
    open() {
      return __async(this, null, function* () {
        if (this.dc.readyState === "open") {
          debug$2("DataChannel open done");
          return;
        }
        const disposers = [];
        const dispose = () => disposers.forEach((d) => d());
        try {
          yield new Promise((resolve, reject) => {
            disposers.push(listen$1(this.dc, "open", () => {
              debug$2("DataChannel event open");
              resolve();
            }));
            disposers.push(listen$1(this.dc, "error", (e) => {
              debug$2("DataChannel event error", e);
              reject(e);
            }));
          });
          debug$2("DataChannel open done");
    ...
    sendRequest() {
      return __async(this, null, function* () {
        var _a;
        debug$2("DataChannel sendRequest");
        ...
        debug$2("DataChannel sendRequest done");
      });
    }
    receiveResponse() {
      return __async(this, null, function* () {
        var _a;
        debug$2("DataChannel receiveResponse");
        const request = this.request;
        ...
        const headers = new Headers(mapObj((_a = respHead.header) != null ? _a : {}, (hs) => hs[0]));
          ...
          start: (ctrl) => {
            (() => __async(this, null, function* () {
              while (true) {
                ...
                if (done2) {
                  debug$2("DataChannel dcReader done");
                  ctrl.close();
                  return;
                }
                ...
                debug$2("received", received, "/", bodyLength);
        ...
        debug$2("DataChannel receiveResponse done with body", resp);
        return resp;
      });
    }
    start() {
        ...
        return new Promise((resolve, reject) => __async(this, null, function* () {
          this.pc.addEventListener("connectionstatechange", (e) => {
            if (this.pc.connectionState === "failed") {
              reject(new Error("PeerConnection failed"));
            }
          });
          try {
            const startAt = Date.now();
            yield this.open();
            debug$2("open cost:", Date.now() - startAt);
  ...
  const debug$1 = (...args) => console.debug("[cdnr/http/webtc/service]", ...args);
  class HoWService {
    constructor() {
    ...
    sendResponse(id, response) {
      return __async(this, null, function* () {
        debug$1("sendResponse in MainPage", id, response);
    ...
    run() {
      this.disposers.push(listen(this.workerContainer, "message", (_0) => __async(this, [_0], function* ({ data }) {
        debug$1("Got message in MainPage", data);
        if (!isRequestMessage(data))
          return;
  ...
  const debug = (...args) => console.debug("[cdnr/proxy/common]", ...args);
  function proxyRequest(client, request) {
    return __async(this, null, function* () {
      return new Promise((resolve) => __async(this, null, function* () {
        var _a, _b, _c, _d, _e, _f, _g;
        const task = client.createTask(request.url);
        ...
        debug("proxyRequest response got", response, [...response.headers.entries()]);
        resolve(response);
      }));
    });
  }
  ...
  function getScriptUrl(scriptUrl, config) {
    return appendQuery(scriptUrl, { [configQueryName]: JSON.stringify(config) });
  }
  ...
  exports2.proxyRequest = proxyRequest;
